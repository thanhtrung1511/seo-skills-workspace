...
|---|---|---|---|---|
| **content-score** | `/content-score` | Nhập URL bài nha khoa → chấm 65 tiêu chí checklist Dr. Care → điểm /100 (≥80 đạt) + nhận xét ưu/nhược/cần cải thiện | ✅ HTTP fetch trang ngoài (CLI/API) | ✅ `references/` + `scripts/` |
...
- ✅ **≥ 1 skill kết nối nền tảng ngoài (API/MCP/CLI)**:
  ...
  - `content-score` kết nối HTTP tới website ngoài để bóc tách tín hiệu SEO (`scripts/extract_seo_signals.py`).
- ✅ **≥ 1 skill có files & folder bên cạnh SKILL.md**:
  - `content-score/` có `references/checklist.md` (65 tiêu chí) và `scripts/extract_seo_signals.py`.
...
2. Gõ `/` để thấy 4 lệnh skill, hoặc gọi trực tiếp, ví dụ:
   - `/content-score https://…` — chấm điểm một bài viết.
...
python3 .claude/skills/content-score/scripts/extract_seo_signals.py "https://..." "keyword chính"
```
...
Checklist chấm điểm trong `content-score` dựa trên bộ tiêu chí content nha khoa
